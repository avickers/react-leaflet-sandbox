export default function fluid(host) {
  let really = 0
  var canvas, ctx;
  const rect = host.shadowRoot.querySelector('.container').getBoundingClientRect()
  const w = host.shadowRoot.querySelector('canvas')
  ...
  */
  var canvas_width = rect.width - rect.width % 30 //Needs to be a multiple of the resolution value below.
  var canvas_height = rect.height - rect.height % 30
  ...
  */
  const initialize = () => {
    ...
    */
    ...
    host.shadowRoot.querySelector('.container').addEventListener("mousemove", mouse_move_handler);
    host.shadowRoot.querySelector('.container').addEventListener("touchmove", touch_move_handler);
  ...
  */
  ...
  function touch_end_handler(e) {
    if (!e.touches) {
      mouse.down = false;
    }//If there are no more touches on the screen, sets "down" to false.
  }
